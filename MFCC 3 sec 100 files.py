...
MODEL_NAME = 'Averaged MFCC Randomized Classifier' #This is the name of the file to which the model will be saved
...
def features_extractor(file):
    ...
    start = start = random.randint(0,LEN_FILES - LEN_DEST)
    ...
    mfccs_scaled_features = np.mean(mfccsN.T,axis=0)
    total_mfccs.append(mfccs_scaled_features)
...
def features_extractor_iter(y):
    ...
    for char in y:
        ...
        X.append(data)
    return np.array(X).reshape(len(y),N_MFCC)
...
def make_model():
    '''
        It creates and returns a deep learning model.
    '''
    model = Sequential()
    #First layer
    model.add(Dense(128,input_shape=(N_MFCC,), kernel_regularizer=regularizers.l2(KERNEL_REGULARIZER)))
    ...
    model.add(Dropout(DROP_OUT))
    #Third layer
    model.add(Dense(32, kernel_regularizer=regularizers.l2(KERNEL_REGULARIZER)))
    model.add(Activation('relu'))
    model.add(Dropout(DROP_OUT))
...
for runs in range(RAND_RUNS):
    ...
    history = model.fit(X_train, y_train, batch_size = N_BATCH_SIZE, epochs = N_EPOCHS, validation_data = (X_validation, y_validation), callbacks=[checkpointer], verbose=0)
