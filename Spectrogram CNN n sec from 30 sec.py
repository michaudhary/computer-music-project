...
MODEL_NAME = 'MFCC Spectrogram CNN Classifier' #This is the name of the file to which the model will be saved
N_FILES = 100 #The amount of files per each genre
N_MFCC = 20 #The amount of MFCCs we will extract
N_SPECTRUM = 128 #The first dimension of the spectrogram
N_DERIV = 1
...
BASE = 0 #The starting numeral part of the file names
KERNEL_REGULARIZER = 0.0001
RAND_RUNS = 1 # The amount of times we train the model
LEN_FILES = 30 # The length of the source files in seconds
LEN_DEST = 30 # Length of the destination files in seconds
FRAME_EDGE = 5 # The number of frames by which we cut down the total frames to account for variations in file length 
DROP_OUT = 0.5
n_frames = 0
n_genres = len(LIST_CATEGORIES)
...
def features_extractor(file):
    ...
    for num in range(LEN_FILES // LEN_DEST):
        ...
        mel_signal = librosa.feature.melspectrogram(y=audio, sr=sample_rate)
        spectrogram = np.abs(mel_signal)
        if (n_frames == 0): #to ensure that n_frames is only set once
            n_frames = spectrogram.shape[1] - FRAME_EDGE
        total_features.append(spectrogram[:,:n_frames])#cut down data to n_frames each
...
def features_extractor_iter_cnn(y):
    ...
    for char in y:
        ...
        X.append(data)
    return np.array(X).reshape(len(y) * (LEN_FILES // LEN_DEST), N_DERIV*  N_SPECTRUM, n_frames, 1)
...
for runs in range(RAND_RUNS):
    #These create y_train, y_validation and y_test from label, which are arrays of the classes of each file used
