...
MODEL_NAME = 'MFCC Basic Model' #This is the name of the file to which the model will be saved
...
DROP_OUT = 0.4 #The dropout regulation
...
LEN_DEST = 1 # Length of destination files in seconds
...
def features_extractor_iter(y):
    ...
    for char in y:
        ...
        X.append(data)
    return np.array(X).reshape(len(y) * (LEN_FILES // LEN_DEST),N_MFCC)
...
def make_model():
    ...
    model = Sequential()
    #First layer
    model.add(Dense(512,input_shape=(N_MFCC,), kernel_regularizer=regularizers.l2(KERNEL_REGULARIZER)))
